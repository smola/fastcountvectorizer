...
void CharNgramCounter::process(const py::str& obj) {
  if (have_fixed_focab()) {
    ...
    if (need_expand_counts()) {
      prefixes->push_back(analyzer->prefix(n, obj));
    }
...
template <class I>
void prefixes_add_csr_matrix(ngram_analyzer* _analyzer, vocab_map& vocab,
  ...
  for (unsigned int i = 0; i < prefixes.size(); i++) {
    ...
    for (unsigned int n = new_max_n; n >= min_n; n--) {
      for (unsigned int start = 0; start < max_n - n; start++) {
        string_with_kind new_term = _analyzer->ngram(prefixes[i], n, start);
